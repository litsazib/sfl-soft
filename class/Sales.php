...
($_POST['bagSize'][$k]!=null && $_POST['bagSize'][$k]!="0") || ($_POST['bagUnitNumber'][$k]!=null && $_POST['bagUnitNumber'][$k]!="0"))){
		...
		public function SalesDelete()
    {
      $this->LoginAllredy(true);
      //print_r($_GET);
      $this->sql("DELETE  FROM  `product_accounts` WHERE `osm_fulll_id`=".$this->DBH->quote($_GET['getID'])."");
   	  $this->sql("DELETE  FROM `product_osm_accounts` WHERE `osm_full_id`=".$this->DBH->quote($_GET['getID'])."");
      $this->sql("DELETE  FROM `customer_accounts` WHERE `ref_osm_full_id`=".$this->DBH->quote($_GET['getID'])."");
